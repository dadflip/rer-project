...
# Backend Node Deployment Script
# Deploys the RER Backend service with environment configuration
...
UNIV_ENV="$BACKEND_DIR/backend.univ.env"
LOCAL_ENV="$BACKEND_DIR/backend.local.env"
...
if [ ! -f "$LOCAL_ENV" ]; then
  ...
  if [ -f "$BACKEND_DIR/../../env-ex/backend.local.env.example" ]; then
    cp "$BACKEND_DIR/../../env-ex/backend.local.env.example" "$LOCAL_ENV"
  else
...
echo "Backend Node deployed successfully"
